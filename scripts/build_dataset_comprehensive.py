...
-------------------------------------------------------------------
CONFIRMED AS THE ONGOING DATA SOURCE. Originally built as a separate,
experimental copy of build_dataset.py; the team has since confirmed
this consolidated schema should be treated as the real, ongoing
source going forward, not the old three-file split (features_v2.tsv
+ features_v3.tsv + audio_analysis.tsv). This script still does NOT
touch or modify build_dataset.py or any of the teammate's own
pipeline files -- it only ever reads copies.

WHY THIS EXISTS
    The pipeline consolidates pacing + escalation/recovery + (most)
    audio features into one file, built directly around the site's
    10 pattern types rather than raw measurement categories. It also:
      ...
      - Does NOT include loudness_oscillation_score, one of the 13
        ...
        STILL AN OPEN GAP as of this writing -- audio_analysis.tsv
        only covers the original 23-video pilot, so this one feature
        stays "not enough data" for most of the corpus until that
        file is expanded.
...
USAGE
    ...
    result before pointing --out at the live assets/js/data.js.
-------------------------------------------------------------------
"""
...
def write_data_js(videos, out_path, coverage, source_label):
    ...
    js = "/**\n * Auto-generated by scripts/build_dataset_comprehensive.py — do not hand-edit.\n */\n\n"
